...
export function HorarioList({ horarios }: { horarios: TransporteHorario[] }) {
  ...
  if (horarios.length === 0) {
    return <p className="text-sm text-[var(--tp-muted-foreground)]">No tienes horarios programados.</p>;
  }
    ...
    <ul className="divide-y divide-[var(--tp-border)]">
      {horarios.map((h) => (
        <li key={h.id} className="flex items-center justify-between gap-3 py-3">
          <div>
            <p className={`font-medium ${!h.activo ? 'text-[var(--tp-muted-foreground)] line-through' : ''}`}>
              {h.hora.slice(0, 5)}
            </p>
