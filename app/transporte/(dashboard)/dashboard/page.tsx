...
import { formatMonto, formatFecha, DIAS_SEMANA, TIPO_LABEL } from '@/lib/transporte/format';
import type { TransporteConfig, TransporteHorario, TransporteMovimiento } from '@/lib/transporte/types';
  ...
  return (
    <div className="space-y-6">
      <Card className="p-6 text-center">
        <p className="text-sm text-[var(--tp-muted-foreground)]">Saldo estimado</p>
        <p className={`mt-1 text-4xl font-black ${saldo < 0 ? 'text-[var(--tp-destructive)]' : ''}`}>
          {formatMonto(saldo, moneda)}
        </p>
        {sinCostoConfigurado && (
          <p className="mt-3 text-sm text-[var(--tp-muted-foreground)]">
            Aún no configuras el costo del viaje.{' '}
            <Link href="/transporte/configuracion" className="text-[var(--tp-primary)] font-semibold hover:underline">
              Configúralo aquí
            </Link>
          </p>
        )}
      ...
      <Card className="p-5 space-y-3">
        <div className="flex items-center justify-between">
          ...
          <Link href="/transporte/horarios" className="text-sm text-[var(--tp-primary)] font-semibold hover:underline">
            Administrar
          </Link>
        </div>
        {!horarios || horarios.length === 0 ? (
          ...
          <ul className="divide-y divide-[var(--tp-border)]">
            {horarios.map((h) => (
      ...
      <Card className="p-5 space-y-3">
        <div className="flex items-center justify-between">
          ...
          <Link href="/transporte/historial" className="text-sm text-[var(--tp-primary)] font-semibold hover:underline">
            Ver historial
          </Link>
        </div>
        {!movimientos || movimientos.length === 0 ? (
          <p className="text-sm text-[var(--tp-muted-foreground)]">Todavía no hay movimientos.</p>
        ) : (
          <ul className="divide-y divide-[var(--tp-border)]">
            {movimientos.map((m) => (
              <li key={m.id} className="flex items-center justify-between py-2 text-sm">
                <div>
                  ...
                  <p className="text-[var(--tp-muted-foreground)]">{formatFecha(m.fecha)}</p>
                </div>
                <span className={m.monto >= 0 ? 'text-[var(--tp-success)] font-semibold' : 'font-semibold'}>
                  {m.monto >= 0 ? '+' : ''}
